...
import listStyles from "./industry-list-images.module.css";
...
export default function IndustriesPage() {
  return <><Header /><main className={styles.scope + " " + listStyles.scope}>
    ...
    <section className="industryList" aria-label="Industry solutions">{industries.map(([name, lede, description, priorities, cta, slug, image], index) => <article key={name}><div className="industryImage"><Image src={image} alt={name} fill sizes="(max-width: 900px) 88vw, 185px" /></div><div className="industryCopy"><div className="industryNumber">0{index + 1}</div><h2>{name}</h2><p className="lede">{lede}</p><p>{description}</p><Link className="industryLink" href={"/industries/" + slug}>{cta} ↗</Link></div><div className="priorities"><div className="kicker">Typical priorities</div><ul>{priorities.map((priority) => <li key={priority}>{priority}</li>)}</ul></div></article>)}</section>
  </main><Footer /></>;
}
